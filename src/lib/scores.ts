export interface ToolScore {
  id: string;
  name: string;
  provider: string;
  category: string;
  performance: number;   // 0–10: benchmark scores, output quality
  value: number;         // 0–10: price-to-performance ratio
  reliability: number;   // 0–10: uptime, stability, vendor risk
  easeOfUse: number;     // 0–10: interface, docs, setup friction
  overall: number;       // weighted: perf 35% + value 30% + reliability 20% + ease 15%
  verdict: string;       // one-line summary
  lastUpdated: string;
}

function calcOverall(p: number, v: number, r: number, e: number) {
  return Math.round((p * 0.35 + v * 0.30 + r * 0.20 + e * 0.15) * 10) / 10;
}

export const ALL_SCORES: ToolScore[] = [
  // ── LLM Models ──
  {
    id: "gpt-4o",
    name: "GPT-4o",
    provider: "OpenAI",
    category: "LLM",
    performance: 9.0,
    value: 8.2,
    reliability: 9.0,
    easeOfUse: 9.5,
    overall: calcOverall(9.0, 8.2, 9.0, 9.5),
    verdict: "Best all-rounder. Unmatched ecosystem and ease of use.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "claude-opus-4",
    name: "Claude Opus 4",
    provider: "Anthropic",
    category: "LLM",
    performance: 9.5,
    value: 7.5,
    reliability: 9.0,
    easeOfUse: 8.5,
    overall: calcOverall(9.5, 7.5, 9.0, 8.5),
    verdict: "Top reasoning quality. Best for complex, high-stakes tasks.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "gemini-2-5-pro",
    name: "Gemini 2.5 Pro",
    provider: "Google",
    category: "LLM",
    performance: 8.8,
    value: 8.5,
    reliability: 8.5,
    easeOfUse: 8.2,
    overall: calcOverall(8.8, 8.5, 8.5, 8.2),
    verdict: "Excellent value. Best choice for Google Workspace teams.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "deepseek-v3",
    name: "DeepSeek V3",
    provider: "DeepSeek",
    category: "LLM",
    performance: 8.5,
    value: 9.5,
    reliability: 6.5,
    easeOfUse: 7.0,
    overall: calcOverall(8.5, 9.5, 6.5, 7.0),
    verdict: "Exceptional value. Strong performance at a fraction of the cost.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "mistral-large",
    name: "Mistral Large",
    provider: "Mistral AI",
    category: "LLM",
    performance: 8.0,
    value: 8.5,
    reliability: 7.5,
    easeOfUse: 7.5,
    overall: calcOverall(8.0, 8.5, 7.5, 7.5),
    verdict: "Strong European alternative with good price and GDPR compliance.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "llama-3-1-405b",
    name: "LLaMA 3.1 405B",
    provider: "Meta",
    category: "LLM",
    performance: 8.5,
    value: 9.5,
    reliability: 6.0,
    easeOfUse: 5.0,
    overall: calcOverall(8.5, 9.5, 6.0, 5.0),
    verdict: "Best open-source model. Free to run, but requires infrastructure.",
    lastUpdated: "2026-04-13",
  },

  // ── Coding Tools ──
  {
    id: "github-copilot",
    name: "GitHub Copilot",
    provider: "GitHub / Microsoft",
    category: "Coding",
    performance: 8.5,
    value: 8.2,
    reliability: 9.0,
    easeOfUse: 9.5,
    overall: calcOverall(8.5, 8.2, 9.0, 9.5),
    verdict: "Best IDE integration. The most frictionless coding assistant available.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "cursor",
    name: "Cursor",
    provider: "Anysphere",
    category: "Coding",
    performance: 9.0,
    value: 8.0,
    reliability: 8.0,
    easeOfUse: 8.5,
    overall: calcOverall(9.0, 8.0, 8.0, 8.5),
    verdict: "Best AI-native editor. Codebase-wide context sets it apart.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "claude-code",
    name: "Claude Code",
    provider: "Anthropic",
    category: "Coding",
    performance: 9.2,
    value: 7.5,
    reliability: 8.5,
    easeOfUse: 8.0,
    overall: calcOverall(9.2, 7.5, 8.5, 8.0),
    verdict: "Best for complex engineering tasks and large refactors.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "windsurf",
    name: "Windsurf",
    provider: "Codeium",
    category: "Coding",
    performance: 8.5,
    value: 8.8,
    reliability: 7.5,
    easeOfUse: 8.5,
    overall: calcOverall(8.5, 8.8, 7.5, 8.5),
    verdict: "Best value coding editor. Strong Cursor alternative at lower price.",
    lastUpdated: "2026-04-13",
  },

  // ── Image Generation ──
  {
    id: "midjourney",
    name: "Midjourney",
    provider: "Midjourney",
    category: "Image",
    performance: 9.5,
    value: 7.5,
    reliability: 8.0,
    easeOfUse: 7.0,
    overall: calcOverall(9.5, 7.5, 8.0, 7.0),
    verdict: "Best image quality available. Discord interface is the main drawback.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "dall-e-3",
    name: "DALL-E 3",
    provider: "OpenAI",
    category: "Image",
    performance: 8.5,
    value: 9.0,
    reliability: 8.5,
    easeOfUse: 9.5,
    overall: calcOverall(8.5, 9.0, 8.5, 9.5),
    verdict: "Most accessible image generator. Included in ChatGPT Plus.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "stable-diffusion",
    name: "Stable Diffusion",
    provider: "Stability AI",
    category: "Image",
    performance: 8.0,
    value: 10.0,
    reliability: 7.0,
    easeOfUse: 5.5,
    overall: calcOverall(8.0, 10.0, 7.0, 5.5),
    verdict: "Best open-source option. Free to run locally with no restrictions.",
    lastUpdated: "2026-04-13",
  },

  // ── Audio / Voice ──
  {
    id: "elevenlabs",
    name: "ElevenLabs",
    provider: "ElevenLabs",
    category: "Audio",
    performance: 9.5,
    value: 7.5,
    reliability: 8.5,
    easeOfUse: 8.5,
    overall: calcOverall(9.5, 7.5, 8.5, 8.5),
    verdict: "Best-in-class voice cloning. Unmatched realism and language support.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "openai-tts",
    name: "OpenAI TTS",
    provider: "OpenAI",
    category: "Audio",
    performance: 8.5,
    value: 9.0,
    reliability: 9.0,
    easeOfUse: 9.0,
    overall: calcOverall(8.5, 9.0, 9.0, 9.0),
    verdict: "Best value TTS. Fast, natural, and priced for scale.",
    lastUpdated: "2026-04-13",
  },

  // ── Cloud Providers ──
  {
    id: "aws-bedrock",
    name: "AWS Bedrock",
    provider: "Amazon",
    category: "Cloud",
    performance: 8.5,
    value: 7.8,
    reliability: 9.5,
    easeOfUse: 7.0,
    overall: calcOverall(8.5, 7.8, 9.5, 7.0),
    verdict: "Most reliable enterprise AI platform. Best for AWS-native teams.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "azure-openai",
    name: "Azure OpenAI",
    provider: "Microsoft",
    category: "Cloud",
    performance: 9.0,
    value: 7.5,
    reliability: 9.5,
    easeOfUse: 8.0,
    overall: calcOverall(9.0, 7.5, 9.5, 8.0),
    verdict: "Best for Microsoft/enterprise shops. GPT-4o with enterprise SLAs.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "vertex-ai",
    name: "Vertex AI",
    provider: "Google Cloud",
    category: "Cloud",
    performance: 8.8,
    value: 8.0,
    reliability: 9.0,
    easeOfUse: 7.5,
    overall: calcOverall(8.8, 8.0, 9.0, 7.5),
    verdict: "Best for Google Cloud teams. Gemini natively integrated.",
    lastUpdated: "2026-04-13",
  },

  // ── New April 2026 models ──
  {
    id: "claude-sonnet-4-6",
    name: "Claude Sonnet 4.6",
    provider: "Anthropic",
    category: "LLM",
    performance: 9.2,
    value: 8.8,
    reliability: 9.0,
    easeOfUse: 8.5,
    overall: calcOverall(9.2, 8.8, 9.0, 8.5),
    verdict: "Best price-performance LLM in 2026. Outperforms GPT-4o at lower cost.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "gemini-2-5-flash",
    name: "Gemini 2.5 Flash",
    provider: "Google",
    category: "LLM",
    performance: 8.5,
    value: 9.8,
    reliability: 8.5,
    easeOfUse: 8.8,
    overall: calcOverall(8.5, 9.8, 8.5, 8.8),
    verdict: "Best value LLM — ultra-fast, incredibly cheap, strong for high-volume tasks.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "gpt-4-1-mini",
    name: "GPT-4.1 Mini",
    provider: "OpenAI",
    category: "LLM",
    performance: 8.2,
    value: 9.5,
    reliability: 9.0,
    easeOfUse: 9.5,
    overall: calcOverall(8.2, 9.5, 9.0, 9.5),
    verdict: "Best budget OpenAI model. Near GPT-4o quality at a fraction of the API cost.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "gpt-4-1",
    name: "GPT-4.1",
    provider: "OpenAI",
    category: "LLM",
    performance: 9.3,
    value: 8.0,
    reliability: 9.2,
    easeOfUse: 9.5,
    overall: calcOverall(9.3, 8.0, 9.2, 9.5),
    verdict: "OpenAI's latest flagship. Best coding performance in the GPT family.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "mistral-large-2",
    name: "Mistral Large 2",
    provider: "Mistral AI",
    category: "LLM",
    performance: 8.5,
    value: 8.8,
    reliability: 8.0,
    easeOfUse: 7.8,
    overall: calcOverall(8.5, 8.8, 8.0, 7.8),
    verdict: "Best European sovereign AI. Strong GDPR compliance and multilingual capabilities.",
    lastUpdated: "2026-04-13",
  },

  // ── Video Generation ──
  {
    id: "sora",
    name: "Sora",
    provider: "OpenAI",
    category: "Video",
    performance: 9.2,
    value: 6.0,
    reliability: 8.0,
    easeOfUse: 8.0,
    overall: calcOverall(9.2, 6.0, 8.0, 8.0),
    verdict: "Best AI video quality. Requires ChatGPT Pro at $200/mo.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "runway-gen3",
    name: "Runway Gen-3",
    provider: "Runway",
    category: "Video",
    performance: 8.5,
    value: 7.5,
    reliability: 8.0,
    easeOfUse: 8.5,
    overall: calcOverall(8.5, 7.5, 8.0, 8.5),
    verdict: "Industry standard for creative professionals. Best camera control and editing tools.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "pika",
    name: "Pika",
    provider: "Pika Labs",
    category: "Video",
    performance: 7.5,
    value: 9.0,
    reliability: 7.5,
    easeOfUse: 9.0,
    overall: calcOverall(7.5, 9.0, 7.5, 9.0),
    verdict: "Best value AI video. Fast, easy, and generous free tier for social content.",
    lastUpdated: "2026-04-13",
  },

  // ── Additional LLMs ──
  {
    id: "grok-3",
    name: "Grok 3",
    provider: "xAI",
    category: "LLM",
    performance: 8.8,
    value: 7.5,
    reliability: 7.5,
    easeOfUse: 8.0,
    overall: calcOverall(8.8, 7.5, 7.5, 8.0),
    verdict: "Best real-time AI with live web and X/Twitter data. Strong reasoning via DeepSearch.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "o1",
    name: "OpenAI o1",
    provider: "OpenAI",
    category: "LLM",
    performance: 9.5,
    value: 6.0,
    reliability: 8.5,
    easeOfUse: 7.5,
    overall: calcOverall(9.5, 6.0, 8.5, 7.5),
    verdict: "Best AI for hard reasoning, math, and science. Expensive and slow but uniquely powerful.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "o3-mini",
    name: "OpenAI o3-mini",
    provider: "OpenAI",
    category: "LLM",
    performance: 8.8,
    value: 8.5,
    reliability: 8.5,
    easeOfUse: 8.0,
    overall: calcOverall(8.8, 8.5, 8.5, 8.0),
    verdict: "Affordable reasoning model. o1-level coding at a fraction of the cost.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "command-r-plus",
    name: "Command R+",
    provider: "Cohere",
    category: "LLM",
    performance: 7.8,
    value: 8.0,
    reliability: 8.0,
    easeOfUse: 7.5,
    overall: calcOverall(7.8, 8.0, 8.0, 7.5),
    verdict: "Best enterprise RAG model. Purpose-built for grounded, citation-based answers.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "phi-4",
    name: "Phi-4",
    provider: "Microsoft",
    category: "LLM",
    performance: 7.5,
    value: 9.5,
    reliability: 7.5,
    easeOfUse: 7.0,
    overall: calcOverall(7.5, 9.5, 7.5, 7.0),
    verdict: "Best small model for on-device AI. Remarkable quality for 14B parameters.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "qwen-2-5",
    name: "Qwen 2.5",
    provider: "Alibaba",
    category: "LLM",
    performance: 8.0,
    value: 9.2,
    reliability: 7.0,
    easeOfUse: 7.0,
    overall: calcOverall(8.0, 9.2, 7.0, 7.0),
    verdict: "Best multilingual open model. Exceptional for Asian languages and cost-sensitive developers.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "llama-3-3-70b",
    name: "LLaMA 3.3 70B",
    provider: "Meta",
    category: "LLM",
    performance: 8.0,
    value: 9.8,
    reliability: 6.5,
    easeOfUse: 5.5,
    overall: calcOverall(8.0, 9.8, 6.5, 5.5),
    verdict: "Best open-source model for local deployment. Near GPT-4o quality at zero API cost.",
    lastUpdated: "2026-04-13",
  },

  // ── Additional Coding Tools ──
  {
    id: "tabnine",
    name: "Tabnine",
    provider: "Tabnine",
    category: "Coding",
    performance: 7.5,
    value: 8.5,
    reliability: 8.5,
    easeOfUse: 9.0,
    overall: calcOverall(7.5, 8.5, 8.5, 9.0),
    verdict: "Best privacy-first code completion. On-premise deployment for enterprise data security.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "replit",
    name: "Replit",
    provider: "Replit",
    category: "Coding",
    performance: 7.8,
    value: 8.0,
    reliability: 8.0,
    easeOfUse: 9.5,
    overall: calcOverall(7.8, 8.0, 8.0, 9.5),
    verdict: "Best cloud IDE for beginners. Full environment + AI in one browser-based platform.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "codeium",
    name: "Codeium",
    provider: "Codeium",
    category: "Coding",
    performance: 7.8,
    value: 9.5,
    reliability: 8.0,
    easeOfUse: 9.0,
    overall: calcOverall(7.8, 9.5, 8.0, 9.0),
    verdict: "Best free code completion. Unlimited AI coding assistance at zero cost for individuals.",
    lastUpdated: "2026-04-13",
  },

  // ── Additional Image Generation ──
  {
    id: "ideogram",
    name: "Ideogram",
    provider: "Ideogram",
    category: "Image",
    performance: 8.0,
    value: 8.8,
    reliability: 8.0,
    easeOfUse: 8.5,
    overall: calcOverall(8.0, 8.8, 8.0, 8.5),
    verdict: "Best AI generator for text inside images. Ideal for logos, posters, and branded content.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "adobe-firefly",
    name: "Adobe Firefly",
    provider: "Adobe",
    category: "Image",
    performance: 7.5,
    value: 8.5,
    reliability: 8.5,
    easeOfUse: 9.0,
    overall: calcOverall(7.5, 8.5, 8.5, 9.0),
    verdict: "Best commercially safe AI images. Trained on licensed content with Adobe IP indemnity.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "flux-1-1-pro",
    name: "Flux 1.1 Pro",
    provider: "Black Forest Labs",
    category: "Image",
    performance: 8.8,
    value: 8.5,
    reliability: 7.5,
    easeOfUse: 7.0,
    overall: calcOverall(8.8, 8.5, 7.5, 7.0),
    verdict: "Best new open-source image model. Near-Midjourney photorealism with open weights.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "leonardo-ai",
    name: "Leonardo AI",
    provider: "Leonardo AI",
    category: "Image",
    performance: 7.8,
    value: 8.5,
    reliability: 7.8,
    easeOfUse: 8.0,
    overall: calcOverall(7.8, 8.5, 7.8, 8.0),
    verdict: "Best AI platform for game assets and character design. Strong for concept art workflows.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "playground-ai",
    name: "Playground AI",
    provider: "Playground AI",
    category: "Image",
    performance: 7.0,
    value: 9.5,
    reliability: 7.5,
    easeOfUse: 8.5,
    overall: calcOverall(7.0, 9.5, 7.5, 8.5),
    verdict: "Best free image generator by volume. 500 images/day free tier is unmatched.",
    lastUpdated: "2026-04-13",
  },

  // ── Additional Audio ──
  {
    id: "murf",
    name: "Murf",
    provider: "Murf AI",
    category: "Audio",
    performance: 8.0,
    value: 7.5,
    reliability: 8.5,
    easeOfUse: 8.8,
    overall: calcOverall(8.0, 7.5, 8.5, 8.8),
    verdict: "Best business TTS studio. Built-in video editor and team collaboration features.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "playht",
    name: "Play.ht",
    provider: "Play.ht",
    category: "Audio",
    performance: 8.2,
    value: 7.5,
    reliability: 8.0,
    easeOfUse: 8.5,
    overall: calcOverall(8.2, 7.5, 8.0, 8.5),
    verdict: "Best voice cloning with developer-friendly API. 800+ voices, 140 languages.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "speechify",
    name: "Speechify",
    provider: "Speechify",
    category: "Audio",
    performance: 7.5,
    value: 7.5,
    reliability: 8.0,
    easeOfUse: 9.0,
    overall: calcOverall(7.5, 7.5, 8.0, 9.0),
    verdict: "Best text-to-speech for reading and learning. Ideal for consuming long-form content at speed.",
    lastUpdated: "2026-04-13",
  },

  // ── Additional Video Generation ──
  {
    id: "kling",
    name: "Kling",
    provider: "Kuaishou",
    category: "Video",
    performance: 8.5,
    value: 8.5,
    reliability: 7.5,
    easeOfUse: 8.0,
    overall: calcOverall(8.5, 8.5, 7.5, 8.0),
    verdict: "Best value AI video with cinematic motion quality. Strong alternative to Runway at lower cost.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "luma-dream-machine",
    name: "Luma Dream Machine",
    provider: "Luma AI",
    category: "Video",
    performance: 8.0,
    value: 8.0,
    reliability: 7.8,
    easeOfUse: 8.5,
    overall: calcOverall(8.0, 8.0, 7.8, 8.5),
    verdict: "Best accessible cinematic video. Photorealistic motion without Sora's $200/mo barrier.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "hailuo",
    name: "Hailuo",
    provider: "MiniMax",
    category: "Video",
    performance: 7.8,
    value: 9.0,
    reliability: 7.5,
    easeOfUse: 8.0,
    overall: calcOverall(7.8, 9.0, 7.5, 8.0),
    verdict: "Best budget AI video. Generous free tier with surprising quality for social content.",
    lastUpdated: "2026-04-13",
  },

  // ── Search / Research AI ──
  {
    id: "perplexity",
    name: "Perplexity",
    provider: "Perplexity AI",
    category: "Search",
    performance: 8.5,
    value: 8.8,
    reliability: 8.5,
    easeOfUse: 9.0,
    overall: calcOverall(8.5, 8.8, 8.5, 9.0),
    verdict: "Best AI search with real-time citations. Go-to for research and fact-checking.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "youcom",
    name: "You.com",
    provider: "You.com",
    category: "Search",
    performance: 7.5,
    value: 9.0,
    reliability: 7.8,
    easeOfUse: 8.5,
    overall: calcOverall(7.5, 9.0, 7.8, 8.5),
    verdict: "Best privacy-first AI search. Free unlimited searches with no user tracking.",
    lastUpdated: "2026-04-13",
  },

  // ── App Builders ──
  {
    id: "lovable",
    name: "Lovable",
    provider: "Lovable",
    category: "AppBuilder",
    performance: 9.0,
    value: 8.5,
    reliability: 8.0,
    easeOfUse: 9.5,
    overall: calcOverall(9.0, 8.5, 8.0, 9.5),
    verdict: "Fastest growing AI app builder. Full-stack apps from text prompts with GitHub sync.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "bolt-new",
    name: "Bolt.new",
    provider: "StackBlitz",
    category: "AppBuilder",
    performance: 8.8,
    value: 8.8,
    reliability: 8.0,
    easeOfUse: 9.0,
    overall: calcOverall(8.8, 8.8, 8.0, 9.0),
    verdict: "Best browser-based full-stack builder. Deploy in one click with zero local setup.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "v0-vercel",
    name: "V0 (Vercel)",
    provider: "Vercel",
    category: "AppBuilder",
    performance: 8.5,
    value: 8.0,
    reliability: 8.5,
    easeOfUse: 9.0,
    overall: calcOverall(8.5, 8.0, 8.5, 9.0),
    verdict: "Best AI UI generator for React/Next.js. Produces production-ready Shadcn components.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "devin",
    name: "Devin",
    provider: "Cognition",
    category: "AppBuilder",
    performance: 8.5,
    value: 7.0,
    reliability: 7.5,
    easeOfUse: 8.0,
    overall: calcOverall(8.5, 7.0, 7.5, 8.0),
    verdict: "First true AI software engineer. Handles end-to-end tasks autonomously with minimal guidance.",
    lastUpdated: "2026-04-13",
  },

  // ── Additional Coding Tools ──
  {
    id: "amazon-q-developer",
    name: "Amazon Q Developer",
    provider: "Amazon",
    category: "Coding",
    performance: 8.0,
    value: 8.5,
    reliability: 9.0,
    easeOfUse: 8.5,
    overall: calcOverall(8.0, 8.5, 9.0, 8.5),
    verdict: "Best coding assistant for AWS developers. Deep AWS service knowledge built in.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "aider",
    name: "Aider",
    provider: "Aider AI",
    category: "Coding",
    performance: 8.8,
    value: 9.5,
    reliability: 7.5,
    easeOfUse: 7.0,
    overall: calcOverall(8.8, 9.5, 7.5, 7.0),
    verdict: "Best open-source CLI coding agent. Pair programs with Claude/GPT-4 at near-zero cost.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "cline",
    name: "Cline",
    provider: "Cline",
    category: "Coding",
    performance: 8.5,
    value: 9.5,
    reliability: 7.8,
    easeOfUse: 8.0,
    overall: calcOverall(8.5, 9.5, 7.8, 8.0),
    verdict: "Best VS Code AI agent extension. Zero API markup — pay only model costs.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "gemini-code-assist",
    name: "Gemini Code Assist",
    provider: "Google",
    category: "Coding",
    performance: 8.0,
    value: 8.8,
    reliability: 8.5,
    easeOfUse: 8.5,
    overall: calcOverall(8.0, 8.8, 8.5, 8.5),
    verdict: "Best coding assistant for Google Cloud teams. Free tier generous, strong Python/Go support.",
    lastUpdated: "2026-04-13",
  },

  // ── AI Agents ──
  {
    id: "openai-operator",
    name: "OpenAI Operator",
    provider: "OpenAI",
    category: "Agents",
    performance: 8.5,
    value: 6.5,
    reliability: 7.5,
    easeOfUse: 8.5,
    overall: calcOverall(8.5, 6.5, 7.5, 8.5),
    verdict: "First mainstream consumer AI agent. Browses the web and completes tasks autonomously.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "manus-ai",
    name: "Manus AI",
    provider: "Monica",
    category: "Agents",
    performance: 8.0,
    value: 7.0,
    reliability: 7.0,
    easeOfUse: 8.0,
    overall: calcOverall(8.0, 7.0, 7.0, 8.0),
    verdict: "Viral autonomous agent. Handles complex multi-step research and task completion.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "n8n",
    name: "n8n",
    provider: "n8n",
    category: "Agents",
    performance: 8.5,
    value: 9.5,
    reliability: 8.5,
    easeOfUse: 7.5,
    overall: calcOverall(8.5, 9.5, 8.5, 7.5),
    verdict: "Best open-source AI workflow automation. Self-hostable with 400+ integrations.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "zapier-ai",
    name: "Zapier AI",
    provider: "Zapier",
    category: "Agents",
    performance: 7.5,
    value: 7.0,
    reliability: 9.0,
    easeOfUse: 9.5,
    overall: calcOverall(7.5, 7.0, 9.0, 9.5),
    verdict: "Best no-code AI automation for non-developers. 7,000+ app integrations out of the box.",
    lastUpdated: "2026-04-13",
  },

  // ── AI Writing Tools ──
  {
    id: "jasper",
    name: "Jasper",
    provider: "Jasper AI",
    category: "Writing",
    performance: 8.0,
    value: 6.5,
    reliability: 8.5,
    easeOfUse: 8.5,
    overall: calcOverall(8.0, 6.5, 8.5, 8.5),
    verdict: "Best AI writing platform for marketing teams. Templates, brand voice, and team collaboration built in.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "grammarly",
    name: "Grammarly",
    provider: "Grammarly",
    category: "Writing",
    performance: 8.0,
    value: 7.5,
    reliability: 9.0,
    easeOfUse: 9.5,
    overall: calcOverall(8.0, 7.5, 9.0, 9.5),
    verdict: "Most widely-used AI writing assistant. 50M+ users. Best for editing, grammar, and tone.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "copy-ai",
    name: "Copy.ai",
    provider: "Copy.ai",
    category: "Writing",
    performance: 7.5,
    value: 8.0,
    reliability: 8.0,
    easeOfUse: 8.8,
    overall: calcOverall(7.5, 8.0, 8.0, 8.8),
    verdict: "Best AI for GTM and sales copy. Workflow automation for B2B content at scale.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "notion-ai",
    name: "Notion AI",
    provider: "Notion",
    category: "Writing",
    performance: 7.5,
    value: 8.5,
    reliability: 8.5,
    easeOfUse: 9.5,
    overall: calcOverall(7.5, 8.5, 8.5, 9.5),
    verdict: "Best AI for knowledge management. Summaries, drafts, and Q&A directly inside your Notion workspace.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "writesonic",
    name: "Writesonic",
    provider: "Writesonic",
    category: "Writing",
    performance: 7.8,
    value: 8.2,
    reliability: 8.0,
    easeOfUse: 8.5,
    overall: calcOverall(7.8, 8.2, 8.0, 8.5),
    verdict: "Best AI for SEO content. Built-in keyword research and on-page optimisation suggestions.",
    lastUpdated: "2026-04-13",
  },

  // ── AI Design & Presentations ──
  {
    id: "canva-ai",
    name: "Canva AI",
    provider: "Canva",
    category: "Design",
    performance: 8.0,
    value: 9.0,
    reliability: 9.0,
    easeOfUse: 9.8,
    overall: calcOverall(8.0, 9.0, 9.0, 9.8),
    verdict: "Most accessible AI design tool. 200M+ users — the default choice for non-designers.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "figma-ai",
    name: "Figma AI",
    provider: "Figma",
    category: "Design",
    performance: 8.5,
    value: 7.5,
    reliability: 9.0,
    easeOfUse: 8.5,
    overall: calcOverall(8.5, 7.5, 9.0, 8.5),
    verdict: "Best AI for professional UI/UX design. Used by 85% of Fortune 500 design teams.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "gamma",
    name: "Gamma",
    provider: "Gamma",
    category: "Design",
    performance: 8.0,
    value: 8.5,
    reliability: 8.5,
    easeOfUse: 9.5,
    overall: calcOverall(8.0, 8.5, 8.5, 9.5),
    verdict: "Best AI presentation builder. Beautiful decks from a prompt in under 60 seconds.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "tome",
    name: "Tome",
    provider: "Tome",
    category: "Design",
    performance: 7.5,
    value: 8.0,
    reliability: 8.0,
    easeOfUse: 9.0,
    overall: calcOverall(7.5, 8.0, 8.0, 9.0),
    verdict: "Best for narrative-driven AI presentations. Cinematic storytelling format unique to Tome.",
    lastUpdated: "2026-04-13",
  },

  // ── New LLMs ──
  {
    id: "gpt-5",
    name: "GPT-5",
    provider: "OpenAI",
    category: "LLM",
    performance: 9.7,
    value: 7.5,
    reliability: 9.2,
    easeOfUse: 9.5,
    overall: calcOverall(9.7, 7.5, 9.2, 9.5),
    verdict: "OpenAI's most capable model. Leads reasoning, coding, and multimodal benchmarks in 2026.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "o3",
    name: "OpenAI o3",
    provider: "OpenAI",
    category: "LLM",
    performance: 9.8,
    value: 5.5,
    reliability: 8.5,
    easeOfUse: 7.5,
    overall: calcOverall(9.8, 5.5, 8.5, 7.5),
    verdict: "Best AI for hard math, science, and coding. Tops every reasoning benchmark — expensive and slow.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "llama-4-scout",
    name: "LLaMA 4 Scout",
    provider: "Meta",
    category: "LLM",
    performance: 8.8,
    value: 9.8,
    reliability: 6.0,
    easeOfUse: 5.5,
    overall: calcOverall(8.8, 9.8, 6.0, 5.5),
    verdict: "Best open-source model with 10M token context. Free to run, industry-leading context length.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "grok-4",
    name: "Grok 4",
    provider: "xAI",
    category: "LLM",
    performance: 9.2,
    value: 7.5,
    reliability: 7.8,
    easeOfUse: 8.5,
    overall: calcOverall(9.2, 7.5, 7.8, 8.5),
    verdict: "xAI's most powerful model. 1M token context, real-time X/web data, and strong reasoning.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "microsoft-copilot",
    name: "Microsoft Copilot",
    provider: "Microsoft",
    category: "LLM",
    performance: 8.5,
    value: 8.0,
    reliability: 9.0,
    easeOfUse: 9.0,
    overall: calcOverall(8.5, 8.0, 9.0, 9.0),
    verdict: "Best AI for Microsoft 365 users. GPT-4o power natively in Word, Excel, Teams, and Outlook.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "gemma-3",
    name: "Gemma 3",
    provider: "Google",
    category: "LLM",
    performance: 7.8,
    value: 9.8,
    reliability: 7.0,
    easeOfUse: 7.0,
    overall: calcOverall(7.8, 9.8, 7.0, 7.0),
    verdict: "Best small open model for on-device AI. Runs on a single GPU with competitive quality.",
    lastUpdated: "2026-04-13",
  },

  // ── New Image Generation ──
  {
    id: "recraft-v4",
    name: "Recraft V4",
    provider: "Recraft",
    category: "Image",
    performance: 9.2,
    value: 8.0,
    reliability: 8.0,
    easeOfUse: 8.5,
    overall: calcOverall(9.2, 8.0, 8.0, 8.5),
    verdict: "Led image generation leaderboards in 2026. Only AI image tool with true SVG export.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "google-imagen-4",
    name: "Google Imagen 4",
    provider: "Google",
    category: "Image",
    performance: 8.8,
    value: 9.2,
    reliability: 9.0,
    easeOfUse: 8.5,
    overall: calcOverall(8.8, 9.2, 9.0, 8.5),
    verdict: "Best value frontier image model. $0.02/image at native 2K resolution via Vertex AI.",
    lastUpdated: "2026-04-13",
  },

  // ── New Audio ──
  {
    id: "cartesia-sonic",
    name: "Cartesia Sonic",
    provider: "Cartesia",
    category: "Audio",
    performance: 9.0,
    value: 8.5,
    reliability: 8.5,
    easeOfUse: 8.0,
    overall: calcOverall(9.0, 8.5, 8.5, 8.0),
    verdict: "Fastest TTS API — 90ms latency. The go-to choice for real-time voice agent builders.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "deepgram",
    name: "Deepgram",
    provider: "Deepgram",
    category: "Audio",
    performance: 9.0,
    value: 8.5,
    reliability: 9.0,
    easeOfUse: 8.5,
    overall: calcOverall(9.0, 8.5, 9.0, 8.5),
    verdict: "Best enterprise STT/TTS API. Sub-300ms latency, 36+ languages, SOC 2 compliant.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "assemblyai",
    name: "AssemblyAI",
    provider: "AssemblyAI",
    category: "Audio",
    performance: 8.8,
    value: 8.5,
    reliability: 9.0,
    easeOfUse: 9.0,
    overall: calcOverall(8.8, 8.5, 9.0, 9.0),
    verdict: "Best speech-to-text API for developers. LeMUR feature adds LLM reasoning over audio.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "openai-whisper",
    name: "OpenAI Whisper",
    provider: "OpenAI",
    category: "Audio",
    performance: 8.5,
    value: 9.8,
    reliability: 7.0,
    easeOfUse: 7.5,
    overall: calcOverall(8.5, 9.8, 7.0, 7.5),
    verdict: "Best open-source transcription model. Free to run locally. Supports 99 languages.",
    lastUpdated: "2026-04-13",
  },

  // ── New Video Generation ──
  {
    id: "veo-3",
    name: "Veo 3",
    provider: "Google",
    category: "Video",
    performance: 9.5,
    value: 7.5,
    reliability: 8.5,
    easeOfUse: 8.0,
    overall: calcOverall(9.5, 7.5, 8.5, 8.0),
    verdict: "Top-ranked AI video model in 2026. Native audio, 4K resolution, unmatched realism.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "heygen",
    name: "HeyGen",
    provider: "HeyGen",
    category: "Video",
    performance: 8.8,
    value: 7.5,
    reliability: 8.5,
    easeOfUse: 9.0,
    overall: calcOverall(8.8, 7.5, 8.5, 9.0),
    verdict: "Best AI avatar video for sales and marketing. Realistic talking-head video in 5 minutes.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "synthesia",
    name: "Synthesia",
    provider: "Synthesia",
    category: "Video",
    performance: 8.5,
    value: 7.0,
    reliability: 9.0,
    easeOfUse: 9.0,
    overall: calcOverall(8.5, 7.0, 9.0, 9.0),
    verdict: "Best enterprise AI video platform. 50M+ users, SOC 2 compliant, 140+ languages.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "wan-2-1",
    name: "Wan 2.1",
    provider: "Alibaba",
    category: "Video",
    performance: 8.2,
    value: 9.8,
    reliability: 7.0,
    easeOfUse: 6.5,
    overall: calcOverall(8.2, 9.8, 7.0, 6.5),
    verdict: "Best open-source video model. Apache 2.0 license — run locally for free with no restrictions.",
    lastUpdated: "2026-04-13",
  },

  // ── New Search AI ──
  {
    id: "chatgpt-search",
    name: "ChatGPT Search",
    provider: "OpenAI",
    category: "Search",
    performance: 8.5,
    value: 8.0,
    reliability: 9.0,
    easeOfUse: 9.5,
    overall: calcOverall(8.5, 8.0, 9.0, 9.5),
    verdict: "OpenAI's answer engine with real-time web results. Built into ChatGPT for seamless search.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "consensus",
    name: "Consensus",
    provider: "Consensus",
    category: "Search",
    performance: 8.5,
    value: 8.0,
    reliability: 8.5,
    easeOfUse: 8.5,
    overall: calcOverall(8.5, 8.0, 8.5, 8.5),
    verdict: "Best AI for academic research. Searches 200M+ scientific papers with consensus scoring.",
    lastUpdated: "2026-04-13",
  },

  // ── Music Generation ──
  {
    id: "suno",
    name: "Suno",
    provider: "Suno",
    category: "Music",
    performance: 8.5,
    value: 9.0,
    reliability: 8.0,
    easeOfUse: 9.5,
    overall: calcOverall(8.5, 9.0, 8.0, 9.5),
    verdict: "Best AI music generator. Full songs with vocals in 30 seconds — no music knowledge needed.",
    lastUpdated: "2026-04-13",
  },
  {
    id: "udio",
    name: "Udio",
    provider: "Udio",
    category: "Music",
    performance: 8.2,
    value: 8.8,
    reliability: 7.8,
    easeOfUse: 8.0,
    overall: calcOverall(8.2, 8.8, 7.8, 8.0),
    verdict: "Best AI music for professionals. More control and higher quality ceiling than Suno.",
    lastUpdated: "2026-04-13",
  },
];

export function getScoresByCategory(category: string) {
  return ALL_SCORES.filter((s) => s.category === category).sort((a, b) => b.overall - a.overall);
}

export function getScoreById(id: string) {
  return ALL_SCORES.find((s) => s.id === id);
}

export function getScoreByName(name: string) {
  return ALL_SCORES.find(
    (s) => s.name.toLowerCase() === name.toLowerCase() ||
           s.name.toLowerCase().includes(name.toLowerCase())
  );
}

export const CATEGORIES = [...new Set(ALL_SCORES.map((s) => s.category))];

export const SCORE_DIMENSIONS = [
  { key: "performance" as const, label: "Performance", color: "bg-blue-500",    desc: "Benchmark scores & output quality" },
  { key: "value"       as const, label: "Value",       color: "bg-emerald-500", desc: "Price-to-performance ratio" },
  { key: "reliability" as const, label: "Reliability", color: "bg-violet-500",  desc: "Uptime, stability & vendor risk" },
  { key: "easeOfUse"   as const, label: "Ease of Use", color: "bg-amber-500",   desc: "Interface, docs & setup friction" },
];
